...
; Permanent fixed variant based on the original A2U12 image:
; - keeps the $208D IRQ-latch fix
; - changes the P8291A poller at $10DF to read ISR2 first, then ISR1
; This ROM started as the validated ISR2-first experiment and was then kept unchanged as the
; permanent fixed image, so no separate experimental duplicate is retained.
; Hardware testing with this combined fix has so far shown no observed stalls in repeated
; *IDN? traffic and in repeated WFA? / FRA... / FRA? loops.
;
...
; - Fixed ROM: calls a helper at $7FCF that reads ISR2 first ($0402), then ISR1 ($0401),
;   then saves ISR1→$2246 and ISR2→$2247 before returning with A/B in the same layout
;   expected by the rest of this routine. Hardware testing with this ordering, together with
;   the $208D byte-only latch fix, has so far eliminated the remaining observed FRA? stalls.
...
10de: 39              RTS
10df: bd ff cf        JSR    $FFCF       ; permanent fix helper: read ISR2 first, then ISR1, save $2246/$2247
10e2: 12              NOP
10e3: 12              NOP
10e4: 12              NOP
...
7fcd: 20 fe           BRA    $7FCD
; Permanent P8291A ISR-order helper — $7FCF (CPU $FFCF).
; Uses unused padding ahead of the vector table.
; Entry/exit contract:
; - reads ISR2 first ($0402), then ISR1 ($0401)
; - stores ISR1→$2246 and ISR2→$2247
; - returns with A=isr1, B=isr2 so $10E5 onward can run unchanged
7fcf: f6 04 02        LDB    $0402
7fd2: b6 04 01        LDA    $0401
7fd5: fd 22 46        STD    $2246
7fd8: 39              RTS
7fd9: 45 ff ff        ; unused padding; byte $45 chosen to preserve ROM checksum
...
7fee: ff ff           ; unused padding bytes before vectors
; =====================================================================================
