...
; - Original firmware reads ISR1/ISR2 together via LDD $0401, saving ISR1→$2246, ISR2→$2247.
...
;   Hardware testing with patched ROMs that keep the $208D byte-only latch fix and change this
;   site to read ISR2 first has so far eliminated the remaining observed FRA? stalls, making
;   this original ISR1-first ordering very likely part of that second failure family.
...
208b: 32 66           LEAS   $6,S
; Clear the "descriptor ready" flag.
; Important race window: this 16-bit read-modify-write only intends to clear high-byte bit 1,
; but it rewrites the low byte as well. If BO IRQ arrives after the LDD and before the STD,
; $7154 sets low-byte bit 0 in RAM, then the resumed STD can write the stale pre-IRQ low byte
; back and lose that IRQ latch. Captured post-first-byte failure traces show IRQ landing here.
; Replacing this with a byte-only LDA/ANDA/STA update eliminated the observed *IDN? failures in
; repeated hardware testing, identifying this as the captured talker-stall mechanism.
208d: fc 22 04        LDD    $2204
2090: 84 fd           ANDA   #$FD        ; clear bit 1 of high byte
2092: c4 ff           ANDB   #$FF
2094: fd 22 04        STD    $2204
2097: 32 62           LEAS   $2,S
2099: 39              RTS
